...
const getApiBaseUrl = (): string => {
  ...
  if (typeof window !== 'undefined') {
    ...
    if (isLocalhost) {
      // Use local backend directly for local development if needed
      // However, we'll generally prefer the proxy routes
      return ''; 
    }
...
// In local development, these point directly to the backend or use proxy routes
